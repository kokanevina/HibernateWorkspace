...
public class EmployeeServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(userAction.equalsIgnoreCase("add")) {
			...
			/*
			 * int empId=Integer.parseInt(request.getParameter("empId")); // existing String
			 * empName=request.getParameter("empName"); double
			 * empSalary=Double.parseDouble(request.getParameter("empSalary")); String
			 * qual=request.getParameter("qual"); Employee emp=new
			 * Employee(empId,empName,empSalary,qual); boolean b=crud.updateEmployee(emp);
			 * if(b) { response.sendRedirect("EmployeeServlet?act=getall"); // fresh new req
			 * to same Servlet, doGet } else { response.sendRedirect("Error.jsp"); }
			 */
		}
	}
